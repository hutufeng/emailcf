...
2. [第二阶段：Cloudflare 凭证与 GitHub CI/CD 自动化部署](#第二阶段cloudflare-凭证与-github-cicd-自动化部署)
...
## 第二阶段：Cloudflare 凭证与 GitHub CI/CD 自动化部署

本项目已包含完整的 GitHub Actions 持续集成流，代码推送后自动部署。

### 1. 获取 Cloudflare 凭证
1. **Account ID**：
   - 登录 [Cloudflare 控制台](https://dash.cloudflare.com/)。
   - 在控制台首页右下角，或进入任意一个 Worker 的控制页面右侧，复制 **账户 ID (Account ID)**。
2. **API Token**：
   - 访问 [API 令牌管理页](https://dash.cloudflare.com/profile/api-tokens)。
   - 点击 **创建令牌 (Create Token)**。
   - 选择使用模板 **“编辑 Cloudflare Workers” (Edit Cloudflare Workers)**。
   - 账户资源和区域资源选择“所有”或你的指定账户，点击继续到审核并创建。
   - 复制生成的令牌字符串（只显示一次），此即 `CLOUDFLARE_API_TOKEN`。

### 2. 在 GitHub 仓库配置 Secrets
1. 浏览器打开你的 GitHub 仓库：  
   `https://github.com/hutufeng/emailcf/settings/secrets/actions`
2. 点击 **New repository secret**，依次添加 4 项：

| Name | Secret Value | 说明 |
| :--- | :--- | :--- |
| `CLOUDFLARE_API_TOKEN` | 刚才创建的 Cloudflare API 令牌 | 用于构建部署与动态抓取 AI 模型 |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare 控制台复制的账户 ID | 确定部署的目标账户 |
| `TG_BOT_TOKEN` | Telegram Bot Token (`789012...`) | 机器人发消息鉴权 |
| `TG_CHAT_ID` | 你的 Telegram 数字 ID (`123456...`) | 消息接收目标窗口 |

3. 添加完毕后，进入仓库 **Actions** 标签页，点击 `Deploy Cloudflare Worker` $\rightarrow$ `Run workflow` 即可完成首次部署。
